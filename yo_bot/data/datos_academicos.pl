...
finales_faltantes_hasta_ahora:-materia(X,Nom,_,_,_),not(final_aprobado(X)),cursada_aprobada(X),writeln(-Nom),fail;nl.

%Imprime las cursadas adeudadas
cursadas_faltantes:-materia(X,Nom,_,_,_),not(cursada_aprobada(X)),writeln(-Nom),fail;nl.

%Imprime los finales adeudados
finales_faltantes:-materia(X,Nom,_,_,_),not(final_aprobado(X)),writeln(-Nom),fail;nl.
